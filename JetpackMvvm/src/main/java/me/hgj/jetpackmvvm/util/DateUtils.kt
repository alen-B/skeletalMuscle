...
object DateUtils {
    fun calculateAge(birthdayDate: Date, currentDate: Date): Int {
        ...
        if (currentCalendar.get(Calendar.MONTH) < birthdayCalendar.get(Calendar.MONTH) || currentCalendar.get(Calendar.MONTH) === birthdayCalendar.get(Calendar.MONTH) && currentCalendar.get(Calendar.DAY_OF_MONTH) < birthdayCalendar.get(Calendar.DAY_OF_MONTH)) {
            age--
        }
        return age
    }
