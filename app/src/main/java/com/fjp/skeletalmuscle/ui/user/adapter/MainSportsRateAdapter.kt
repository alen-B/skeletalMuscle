...
class MainSportsRateAdapter(data: ArrayList<MainSports>, var defaultIndex: Int) : BaseQuickAdapter<MainSports, BaseViewHolder>(R.layout.item_main_sports_rate, data) {
    ...
    override fun convert(holder: BaseViewHolder, item: MainSports) {
        if (item.type == SportsType.HIGH_KNEE) {
            ...
            val totalTime = DateTimeUtil.formatTime(item.sports.time)
            holder.setText(R.id.warmupTimeMinTv, DateTimeUtil.formatTime((item.sports.warmupTime).toLong()))
            holder.setText(R.id.fatBurningTimeMinTv, item.sports.fatBurningTime)
            holder.setText(R.id.cardioTimeTotalMinTv, item.sports.cardioTime)
            holder.setText(R.id.breakTimeTotalMinTv, item.sports.breakTime)
