...
class MainSportsRateAdapter(data: ArrayList<MainSports>, var defaultIndex: Int) : BaseQuickAdapter<MainSports, BaseViewHolder>(R.layout.item_main_sports_rate, data) {
    init {
    ...
    override fun convert(holder: BaseViewHolder, item: MainSports) {
        if (item.type == SportsType.HIGH_KNEE) {
            ...
            holder.getView<RoundedProgressBar>(R.id.warmupTimePB).setProgressPercentage((item.sports.warmupTime / (item.sports.time / 1000) * 100).toDouble(), true)
            holder.getView<RoundedProgressBar>(R.id.fatBurningTimePb).setProgressPercentage((item.sports.fatBurningTime / (item.sports.time / 1000) * 100).toDouble(), true)
            holder.getView<RoundedProgressBar>(R.id.cardioTimePb).setProgressPercentage((item.sports.cardioTime / (item.sports.time / 1000) * 100).toDouble(), true)
            holder.getView<RoundedProgressBar>(R.id.breakTimePB).setProgressPercentage((item.sports.breakTime / (item.sports.time / 1000) * 100).toDouble(), true)
            val totalTime = DateTimeUtil.formatTime(item.sports.time / 1000)
            holder.setText(R.id.warmupTimeMinTv, DateTimeUtil.formatTime((item.sports.warmupTime / 1000).toLong()))
            holder.setText(R.id.fatBurningTimeMinTv, item.sports.fatBurningTime / 1000)
            holder.setText(R.id.cardioTimeTotalMinTv, item.sports.cardioTime / 1000)
            holder.setText(R.id.breakTimeTotalMinTv, item.sports.breakTime / 1000)
            ...
            holder.setText(R.id.sportRateTv, "0")
